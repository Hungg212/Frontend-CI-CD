  ...
  const onSubmit = async (data: RegisterData) => {
    ...
    if (result.success) {
      pushNotification({
        ...
        message: 'Chào mừng bạn đến với Coffee Home Blend!',
      });
      navigate('/profile');
    } else {
      pushNotification({
        ...
        message: result.message ?? 'Lỗi đăng ký',
      });
    }
  };
...
const PasswordStrengthIndicator: React.FC<{ password: string }> = ({ password }) => {
  ...
  return (
    <div className="mt-1.5">
      <div className="h-1 overflow-hidden rounded-full bg-stone-200 dark:bg-zinc-700">
